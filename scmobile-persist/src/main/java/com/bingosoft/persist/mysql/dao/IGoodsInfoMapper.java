...
public interface IGoodsInfoMapper {
	...
	@Select("select goods_id goodsId,goods_name goodsName,goods_price goodsPrice,is_recommend isRecommend,goods_desc goodsDesc,goods_note goodsNote from goods_info where category_id = #{categoryId} and goods_status = 1 order by sort_order")
	...
	@Select("select category_id categoryId,goods_name goodsName,goods_title goodsTitle,goods_image goodsImage,goods_desc goodsDesc,goods_note goodsNote,keywords,goods_price goodsPrice,fee_code feeCode from goods_info where goods_id = #{goodsId} and goods_status = 1")
